...
import DONATEINFOS from '@koniverse/extension-koni-base/api/donate';
...
class InputReceive extends React.PureComponent<Props, State> {
  ...
  public override render (): React.ReactNode {
    ...
    return (
      <div className={className}>
        <Dropdown
          className={`ui--InputAddress${hideAddress ? ' hideAddress' : ''}`}
        ...
        {this.state.projectLink && (
          <a
            ...
            target='_blank'
          >
            <span>{this.state.projectLink}</span>
          </a>
        )
        }
      </div>
  ...
  private getFiltered (): Option[] {
    const { filter, optionsAll } = this.props;

    return optionsAll ? dedupe(optionsAll.recent).filter(({ value }) => !filter || (!!value && filter.includes(value))) : [];
  }

  private onChange = (address: string): void => {
    const { filter, onChange, type } = this.props;
    const selectedProject = this.getFiltered().find((p) => p.key === address);

    this.setState({ projectLink: selectedProject?.link });
...
const ExportedComponent = withMulti(
  styled(InputReceive)(({ theme }: ThemeProps) => `
  ...
  withObservable(keyring.keyringOption.optionsSubject, {
    ...
    transform: (optionsAll: KeyringOptions): Record<string, (Option | React.ReactNode)[]> =>
      Object.entries(optionsAll).reduce((result: Record<string, (Option | React.ReactNode)[]>, [type, options]): Record<string, (Option | React.ReactNode)[]> => {
        const projects = Object.values(DONATEINFOS) as Option[];
        const allOptions = projects.concat(options);

        result[type] = allOptions.map((option): Option | React.ReactNode =>
          option.value === null
            ? createHeader(option)
            : createItem(option)
        );
