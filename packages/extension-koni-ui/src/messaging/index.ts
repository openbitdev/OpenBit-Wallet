...
export async function subscribeTransactionHistory (accountGroupId: string, chain: string, callback: (items: TransactionHistoryItem[]) => void): Promise<ResponseSubscribeHistory> {
  return sendMessage('pri(transaction.history.subscribe)', { accountGroupId, chain }, callback);
}
