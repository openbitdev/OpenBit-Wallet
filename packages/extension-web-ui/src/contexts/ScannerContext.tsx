...
import { MULTIPART, SCANNER_QR_STEP } from '@subwallet/extension-web-ui/constants';
import { useTranslation } from '@subwallet/extension-web-ui/hooks';
import { parseEVMTransaction, parseSubstrateTransaction, qrSignEvm, qrSignSubstrate } from '@subwallet/extension-web-ui/messaging';
import { RootState } from '@subwallet/extension-web-ui/stores';
import { CompletedParsedData, EthereumParsedData, MessageQRInfo, MultiFramesInfo, QrInfo, SubstrateCompletedParsedData, SubstrateMessageParsedData, SubstrateTransactionParsedData, TxQRInfo } from '@subwallet/extension-web-ui/types/scanner';
import { constructDataFromBytes, encodeNumber, findAccountByAddress, getNetworkJsonByInfo, isEthereumCompletedParsedData, isSubstrateMessageParsedData } from '@subwallet/extension-web-ui/utils';
  ...
  const _setDataToSign = useCallback((signRequest: SubstrateMessageParsedData | EthereumParsedData): MessageQRInfo => {
    ...
    if (isSubstrateMessageParsedData(signRequest)) {
      if (signRequest.data.crypto !== 'sr25519') {
        throw new Error(t('OpenBit only supports accounts using sr25519 crypto'));
      }
