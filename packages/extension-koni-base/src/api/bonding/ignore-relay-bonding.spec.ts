...
describe('test DotSama APIs', () => {
  test('test get Validator', async () => {
    ...
    for (const item of eraStakers) {
      ...
      // @ts-ignore
      const totalStakeString = parseBalanceString(PREDEFINED_NETWORKS.aleph.decimals, parsedTotalStake, PREDEFINED_NETWORKS.aleph.nativeToken);
      // @ts-ignore
      const ownStakeString = parseBalanceString(PREDEFINED_NETWORKS.aleph.decimals, parsedOwnStake, PREDEFINED_NETWORKS.aleph.nativeToken);
      // @ts-ignore
      const otherStakeString = parseBalanceString(PREDEFINED_NETWORKS.aleph.decimals, otherStake, PREDEFINED_NETWORKS.aleph.nativeToken);
    ...
    const stakedReturn = calculateChainStakedReturn(inflation, totalEraStake, parsedTotalIssuance, 'aleph');
  ...
  test('check bonded before', async () => {
    const provider = new WsProvider(getCurrentProvider(PREDEFINED_NETWORKS.alephTest), DOTSAMA_AUTO_CONNECT_MS);
    const api = new ApiPromise({ provider });
    const apiPromise = await api.isReady;
