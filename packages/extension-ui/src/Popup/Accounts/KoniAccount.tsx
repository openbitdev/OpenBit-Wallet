...
import { Dropdown, Link, MenuDivider } from '../../components';
import useGenesisHashOptions from '../../hooks/useGenesisHashOptions';
import useTranslation from '../../hooks/useTranslation';
import { tieAccount } from '../../messaging';
import KoniAddress from "@polkadot/extension-ui/components/KoniAddress";
...
function KoniAccount ({ closeSetting, address, className, genesisHash, isExternal, isHardware, isHidden, name, parentName, suri, type }: Props): React.ReactElement<Props> {
  const { t } = useTranslation();
  const [{ isEditing, toggleActions }, setEditing] = useState<EditState>({ isEditing: false, toggleActions: 0 });
  const [editedName] = useState<string | undefined | null>(name);
  ...
  return (
    <div className={className} onClick={closeSetting}>
      <KoniAddress
        actions={_actions}
        address={address}
        className='address'
...
export default styled(KoniAccount)(({ theme }: ThemeProps) => `
  .address {
