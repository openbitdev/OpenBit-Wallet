...
exports.BtcSizeFeeEstimator = void 0;
var _bignumber = _interopRequireDefault(require("bignumber.js"));
// Copyright 2017-2022 @subwallet/keyring
// SPDX-License-Identifier: Apache-2.0

/* eslint-disable camelcase */

// https://github.com/leather-wallet/extension/blob/dev/src/app/common/transactions/bitcoin/fees/btc-size-fee-estimator.ts
...
class BtcSizeFeeEstimator {
  ...
  PUBKEY_SIZE = 33;
  SIGNATURE_SIZE = 72;
  SUPPORTED_INPUT_SCRIPT_TYPES = ['p2pkh', 'p2sh', 'p2sh-p2wpkh', 'p2sh-p2wsh', 'p2wpkh', 'p2wsh', 'p2tr'];
  defaultParams = {
    input_count: 0,
    input_script: 'p2wpkh',
    input_m: 0,
  ...
  getSizeOfletInt(length) {
    if (length < 253) {
      ...
      return 5;
    } else if (new _bignumber.default(length).isLessThan('18446744073709551615')) {
      return 9;
    } else {
  ...
  getTxOverheadVBytes(input_script, input_count, output_count) {
    ...
    return 4 +
    // nVersion
    this.getSizeOfletInt(input_count) +
    // number of inputs
    this.getSizeOfletInt(output_count) +
    // number of outputs
    4 +
    // nLockTime
    witness_vbytes;
  ...
  prepareParams(opts) {
    ...
    if (!Number.isInteger(input_count) || input_count < 0) {
      throw new Error('expecting positive input count, got: '.concat(input_count.toString()));
    }
  ...
  getSizeBasedOnInputType() {
    ...
    switch (this.params.input_script) {
      ...
      case 'p2wpkh':
        ...
        break;
      case 'p2tr':
        // Only consider the cooperative taproot signing path assume multisig is done via aggregate signatures
      ...
      case 'p2sh':
        {
          redeemScriptSize = 1 +
          // OP_M
          this.params.input_n * (1 + this.PUBKEY_SIZE) +
          // OP_PUSH33 <pubkey>
          1 +
          // OP_N
          1; // OP_CHECKMULTISIG
          const scriptSigSize = 1 +
          // size(0)
          this.params.input_m * (1 + this.SIGNATURE_SIZE) +
...
exports.BtcSizeFeeEstimator = BtcSizeFeeEstimator;