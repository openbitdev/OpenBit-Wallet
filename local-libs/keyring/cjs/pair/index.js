...
var _bip = require("@polkadot/util-crypto/mnemonic/bip39");
...
const ECPair = (0, _ecpair.ECPairFactory)(ecc);
const SIG_TYPE_NONE = new Uint8Array();
const toXOnly = pubKey => pubKey.length === 32 ? pubKey : pubKey.slice(1, 33);
function isEmpty(value) {
  return value === null || !value || (0, _util.u8aEmpty)(value);
}
...
function createPair(_ref, _ref2) {
  ...
  const decodePkcs8 = (passphrase, userEncoded) => {
    ...
    if (decoded.secretKey.length === 64) {
      ...
      const pair = _utils.TYPE_FROM_SEED[type](decoded.secretKey);
  ...
  const encodeAddress = () => {
    ...
    switch (type) {
      case 'ethereum':
        return (0, _utilCrypto.ethereumEncode)(raw);
      case 'bitcoin-44':
      case 'bittest-44':
  ...
  const sign = function (message) {
    ...
    return (0, _util.u8aConcat)(options.withType ? _utils.TYPE_PREFIX[type] : SIG_TYPE_NONE, _utils.TYPE_SIGNATURE[type]((0, _util.u8aToU8a)(message), {
      publicKey,
      secretKey
    }));
  };
  return {
    ...
    get evm() {
      ...
      return {
        signMessage: async (payload, method) => {
          ...
          const privateKey = (0, _util.hexStripPrefix)((0, _util.u8aToHex)(secretKey));
          const address = encodeAddress();
          const simpleKeyring = new _ethSimpleKeyring.default([privateKey]);
          switch (method) {
            case 'eth_sign':
        ...
        signTransaction: transaction => {
          ...
          return (0, _util.hexAddPrefix)((0, _util.u8aToHex)(transaction.sign(secretKey).serialize()));
        },
        derive: (index, meta) => {
          if (isEmpty(entropy)) {
          ...
          const phrase = (0, _bip.entropyToMnemonic)(entropy);
          const seed = (0, _utilCrypto.mnemonicToLegacySeed)(phrase, '', false, 64);
          const derivePathFunction = (0, _utils.getDerivePath)(type);
          const derivePath = derivePathFunction(index);
          const derived = (0, _utilCrypto.hdEthereum)(seed, derivePath);
          return createPair({
            toSS58,
            type
          }, derived, meta, null);
    ...
    get substrate() {
      ...
      return {
        derive: (suri, meta) => {
          ...
          const derived = (0, _utils.keyFromPath)({
            publicKey,
            secretKey
          }, path, type);
          return createPair({
    ...
    get bitcoin() {
      ...
      return {
        derive: (index, meta) => {
          ...
          const seed = (0, _utilCrypto.mnemonicToLegacySeed)(phrase, '', false, 64);
          const derivePathFunction = (0, _utils.getDerivePath)(type);
          const derived = (0, _utilCrypto.hdEthereum)(seed, derivePathFunction(index));
          return createPair({
            toSS58,
            type
          }, derived, meta, null);
    ...
    exportMnemonic: passphrase => {
      ...
      return (0, _bip.entropyToMnemonic)(entropy);
    },
    lock: () => {
      secretKey = new Uint8Array();
    },
    ...
    toJson: passphrase => {
      ...
      const address = ['ecdsa', 'ethereum'].includes(type) ? publicKey.length === 20 ? (0, _util.u8aToHex)(publicKey) : (0, _util.u8aToHex)((0, _utilCrypto.secp256k1Compress)(publicKey)) : ['bitcoin-44', 'bitcoin-84', 'bitcoin-86', 'bittest-44', 'bittest-84', 'bittest-86'].includes(type) ? (0, _util.u8aToHex)(publicKey) : encodeAddress();
    ...
    verify: (message, signature, signerPublic) => {
      return (0, _utilCrypto.signatureVerify)(message, signature, _utils.TYPE_ADDRESS[type]((0, _util.u8aToU8a)(signerPublic))).isValid;
    },
    vrfSign: (message, context, extra) => {
      ...
      const proof = _utils.TYPE_SIGNATURE[type]((0, _util.u8aToU8a)(message), {
        publicKey,
        secretKey
      });
    ...
    vrfVerify: (message, vrfResult, signerPublic, context, extra) => {
      ...
      const result = (0, _utilCrypto.signatureVerify)(message, (0, _util.u8aConcat)(_utils.TYPE_PREFIX[type], vrfResult.subarray(32)), _utils.TYPE_ADDRESS[type]((0, _util.u8aToU8a)(signerPublic)));
