...
exports.Keyring = exports.BIP_PROTOCOLS = void 0;
...
const HASH_ROUND = 6;
const BIP_PROTOCOLS = ['ethereum', 'bitcoin-44', 'bitcoin-84', 'bitcoin-86', 'bittest-44', 'bittest-84', 'bittest-86'];
exports.BIP_PROTOCOLS = BIP_PROTOCOLS;
class Keyring {
  #pairs;
  #type;
  #masterPassword;
  #passwordHash;
  #ss58;
  decodeAddress = _utils.decodeAddress;
  constructor() {
  ...
  addFromSeed(seed) {
    ...
    return this.addPair((0, _pair.createPair)({
      toSS58: this.encodeAddress,
      type
    }, _utils.TYPE_FROM_SEED[type](seed), meta, null));
  }
  ...
  createFromUri(_suri) {
    ...
    if (isPhraseHex) {
      ...
      if ([12, 15, 18, 21, 24].includes(parts.length)) {
        seed = BIP_PROTOCOLS.includes(type) ? (0, _utilCrypto.mnemonicToLegacySeed)(phrase, '', false, 64) : (0, _utilCrypto.mnemonicToMiniSecret)(phrase, password);
      } else {
        if (phrase.length > 32) {
  ...
  encodeAddress = (address, ss58Format) => {
    // use switch
    return this.type === 'ethereum' ? (0, _utilCrypto.ethereumEncode)(address) : (0, _utils.encodeAddress)(address, ss58Format === undefined ? this.#ss58 : ss58Format, this.type);
  };
  ...
  doHashPassword(passphrase) {
    if (!passphrase) {
      ...
      const salt = _bcryptjs.default.genSaltSync(HASH_ROUND);
      return _bcryptjs.default.hashSync(passphrase, salt);
    }
  }
  decodePkcs8 = (passphrase, passwordHash) => {
    const newHash = this.doHashPassword(passphrase);
    if (newHash && (!passwordHash || passphrase && _bcryptjs.default.compareSync(passphrase, passwordHash))) {
